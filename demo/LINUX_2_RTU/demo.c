...
main( void )
{
    ...
    else if( eMBInit( &xInstance1.xInstance, &xCallbacks1, MB_RTU, 0x0A, 4, 38400, MB_PAR_EVEN ) != MB_ENOERR )
    {
        fprintf( stderr, "%s: can't initialize modbus stack 1!\n", PROG );
        iExitCode = EXIT_FAILURE;
    }
    else if( eMBInit( &xInstance2.xInstance, &xCallbacks2, MB_RTU, 0x0A, 6, 38400, MB_PAR_EVEN ) != MB_ENOERR )
    {
        fprintf( stderr, "%s: can't initialize modbus stack 2!\n", PROG );
        iExitCode = EXIT_FAILURE;
    }
